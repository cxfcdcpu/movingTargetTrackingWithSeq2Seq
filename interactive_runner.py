...
t_sol = SubprocessThread(sol_args, stderr_prefix="  C++ cuda: ")
t_judge = SubprocessThread(
    judge_args,
    stdin_pipe=t_sol.p.stdout,
    stdout_pipe=t_sol.p.stdin,
    stderr_prefix="Python training: ")
t_sol.start()
t_judge.start()
t_sol.join()
t_judge.join()
...
print()
print("Python training return code:", t_judge.return_code)
if t_judge.error_message:
  print("Python training error message:", t_judge.error_message)

print("C++ cuda return code:", t_sol.return_code)
if t_sol.error_message:
  print("C++ cuda error message:", t_sol.error_message)

if t_sol.return_code:
  print("A C++ finishing with exit code other than 0 (without exceeding "
        "time or memory limits) would be interpreted as a Runtime Error "
        "in the system.")
elif t_judge.return_code:
  print("A C++ finishing with exit code 0 (without exceeding time or "
        "memory limits) and a python finishing with exit code other than 0 "
        "would be interpreted as a Wrong Answer in the system.")
else:
  print("A C++ and python both finishing with exit code 0 (without "
